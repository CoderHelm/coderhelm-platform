...
export class EmailStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: EmailStackProps) {
    ...
    new ses.EmailIdentity(this, "DomainIdentity", {
      identity: ses.Identity.domain("d3ftly.com"),
    ...
    new ses.CfnTemplate(this, "WelcomeTemplate", {
      template: {
        ...
        subjectPart: "Welcome to d3ftly — your AI coding agent is ready",
    ...
    new ses.CfnTemplate(this, "RunCompleteTemplate", {
      template: {
        ...
        subjectPart: "d3ftly completed: {{title}}",
    ...
    new ses.CfnTemplate(this, "RunFailedTemplate", {
      template: {
        ...
        subjectPart: "d3ftly failed: {{title}}",
    ...
    new ses.CfnTemplate(this, "WeeklySummaryTemplate", {
      template: {
        ...
        subjectPart: "Your d3ftly weekly summary",
    ...
    new ses.CfnTemplate(this, "PaymentReceiptTemplate", {
      template: {
        ...
        subjectPart: "d3ftly payment receipt — ${{amount}}",
    ...
    new ses.CfnTemplate(this, "PaymentFailedTemplate", {
      template: {
        ...
        subjectPart: "d3ftly payment failed — action required",
    ...
    new ses.CfnTemplate(this, "SubscriptionCancelledTemplate", {
      template: {
        ...
        subjectPart: "Your d3ftly subscription has been cancelled",
    ...
    new ses.CfnTemplate(this, "InvoiceReadyTemplate", {
      template: {
        ...
        subjectPart: "d3ftly invoice #{{invoice_number}} — ${{amount}}",
    ...
    const sesPolicy = new iam.PolicyStatement({
      ...
      conditions: {
        StringEquals: {
          "ses:FromAddress": `notifications@d3ftly.com`,
        },
      },
    });
...
<tr><td style="padding:32px 40px 24px">
  <img src="https://d3ftly.com/favicon.svg" width="32" height="32" alt="d3ftly" style="display:block;margin-bottom:24px">
  ${body}
  <p style="color:#666;font-size:12px;margin-top:32px;padding-top:16px;border-top:1px solid #222">
    You're receiving this because you have notifications enabled for your d3ftly account.<br>
    <a href="https://app.d3ftly.com/dashboard/settings" style="color:#888">Manage notification preferences</a> · <a href="https://d3ftly.com/privacy" style="color:#888">Privacy</a>
  </p>
</td></tr>
</table>
</td></tr>
...
const WELCOME_HTML = EMAIL_WRAPPER(`
  <h1 style="color:#fff;font-size:24px;margin:0 0 16px">Welcome to d3ftly</h1>
  <p style="color:#ccc;font-size:15px;line-height:1.6">
    Your AI coding agent is ready. d3ftly is now installed on <strong style="color:#fff">{{org}}</strong>
    with access to <strong style="color:#fff">{{repo_count}}</strong> repositories.
  </p>
  <p style="color:#ccc;font-size:15px;line-height:1.6">
    To get started, assign an issue to <code style="background:#1a1a1a;padding:2px 6px;border-radius:4px;color:#4ade80">d3ftly[bot]</code>
    or add the <code style="background:#1a1a1a;padding:2px 6px;border-radius:4px;color:#4ade80">d3ftly</code> label.
  </p>
  <a href="https://app.d3ftly.com/dashboard" style="display:inline-block;background:#fff;color:#000;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600;margin-top:16px">Open Dashboard</a>
`);

const WELCOME_TEXT = `Welcome to d3ftly!

Your AI coding agent is now installed on {{org}} with access to {{repo_count}} repositories.

To get started, assign an issue to d3ftly[bot] or add the "d3ftly" label.

Dashboard: https://app.d3ftly.com/dashboard

Manage notifications: https://app.d3ftly.com/dashboard/settings`;
...
const RUN_COMPLETE_HTML = EMAIL_WRAPPER(`
  ...
  <a href="https://app.d3ftly.com/dashboard/runs/{{run_id}}" style="display:inline-block;background:#fff;color:#000;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;margin-top:8px">View Run</a>
`);
...
View run: https://app.d3ftly.com/dashboard/runs/{{run_id}}`;
...
const WEEKLY_SUMMARY_HTML = EMAIL_WRAPPER(`
  ...
  <a href="https://app.d3ftly.com/dashboard" style="display:inline-block;background:#fff;color:#000;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;margin-top:24px">Open Dashboard</a>
`);
...
const PAYMENT_FAILED_HTML = EMAIL_WRAPPER(`
  ...
  <a href="https://app.d3ftly.com/dashboard/billing" style="display:inline-block;background:#fff;color:#000;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;margin-top:8px">Update Payment Method</a>
`);
...
https://app.d3ftly.com/dashboard/billing`;
...
const SUBSCRIPTION_CANCELLED_HTML = EMAIL_WRAPPER(`
  ...
  <p style="color:#ccc;font-size:15px;line-height:1.6;margin:0 0 16px">
    Your d3ftly subscription has been cancelled. You'll continue to have access until <strong style="color:#fff">{{access_until}}</strong>.
  </p>
  <p style="color:#ccc;font-size:15px;line-height:1.6">
    After that date, d3ftly will stop processing new tickets for your repositories.
  </p>
  <p style="color:#888;font-size:14px;margin-top:16px">
    Changed your mind? You can resubscribe at any time from your dashboard.
  </p>
  <a href="https://app.d3ftly.com/dashboard/billing" style="display:inline-block;background:#fff;color:#000;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;margin-top:8px">Resubscribe</a>
`);

const SUBSCRIPTION_CANCELLED_TEXT = `Subscription cancelled

Your d3ftly subscription has been cancelled. You'll have access until {{access_until}}.
After that, d3ftly will stop processing new tickets.

Resubscribe: https://app.d3ftly.com/dashboard/billing`;
